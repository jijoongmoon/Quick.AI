...
/**
 * @brief Visibility attribute used on the few classes whose typeinfo has
 * to be identical on both sides of the nntrainer <-> plugin DSO split
 * (libnntrainer.so vs libqnn_context.so, libtflite_context.so, ...).
 *
 * On Android NDK the default compile is effectively hidden-visibility
 * for vague-linkage symbols (typeinfo, vtables), so without this the
 * RTTI symbols for Context / ContextData / ContextPluggable exist as
 * *separate* hidden copies inside each DSO. The C++ runtime then sees
 * two distinct type_info objects for the same namespace-qualified name
 * and throws std::bad_cast out of any cross-DSO dynamic_cast<T&> —
 * which on Android happens inside dlopen while the plugin's own
 * static initializers are running, so the failure surfaces as a
 * SIGABRT/std::bad_cast leaking out of dlopen itself.
 *
 * Forcing default visibility on the base classes makes their typeinfo
 * a *single* exported symbol shared across DSOs, which is what the
 * ABI assumes.
 */
#ifndef NNTRAINER_PUBLIC
#if defined(_WIN32)
#define NNTRAINER_PUBLIC
#else
#define NNTRAINER_PUBLIC __attribute__((visibility("default")))
#endif
#endif
...
class NNTRAINER_PUBLIC Context {
public:
 ...
 *
 */
typedef struct NNTRAINER_PUBLIC {
